...
class DiscoveryRepository {
  ...
  Future<List<UserProfile>> loadDiscoveryProfiles({
    ...
    try {
      final result = await _client.rpc('get_nearby_profiles', params: {
        'p_user_id': userId,
        'p_limit': limit,
        'p_offset': offset,
  ...
  Future<UserProfile?> fetchFullProfile(String userId, String currentUserId) async {
    try {
      final profile = await _client
          .from('profiles')
          .select('id, display_name, gender, city, locality, country, bio, relationship_intent, verification_status, is_photo_verified, trust_score, last_active_at')
          .eq('id', userId)
          .maybeSingle();
      ...
      final interests = await _fetchUserInterests(userId);
      final commonInterests = await _fetchCommonInterests(userId, currentUserId);
  ...
  Future<List<String>> _fetchUserInterests(String userId) async {
    try {
      final result = await _client.rpc('get_user_interests', params: {
        'p_user_id': userId,
      });

      if (result == null) return [];
      return (result as List).map((row) => row['name'] as String? ?? '').toList();
    } catch (e) {
      return [];
    }
  }
  ...
  Future<List<String>> _fetchCommonInterests(
    ...
    try {
      ...
      final otherResult = await _client.rpc('get_user_interests', params: {
      ...
      return userInterests.intersection(otherInterests).toList()
        ..sort();
    } catch (e) {
      return [];
    }
  ...
  Future<void> updateLocation({
    ...
    try {
      ...
      final approximate = LocationService.toApproximateCoordinates(lat, lon, 2.0);
      ...
      await _client.from('user_location_buckets').upsert({
        'user_id': userId,
        'geohash_7': geohash,
        'city': city,
        'locality': locality,
  ...
  Future<void> computeCrossedPaths(
    ...
    try {
      await _client.rpc('compute_crossed_paths', params: {
        'p_user_id': userId,
        'p_geohash': geohash,
        'p_city': city,
        'p_locality': locality,
