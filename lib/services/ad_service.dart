...
class AdService extends ChangeNotifier {
  ...
  bool _adTimerStarted = false;
  int _activeDiscoverySeconds = 0;
  bool _adEligible = false;
  bool _adDisplayed = false;
  String? _adId;
  String? _adImpressionId;
  bool _adLoading = false;
  String? _adError;
  bool _paused = false;
  Timer? _timer;

  bool get adTimerStarted => _adTimerStarted;
  int get activeDiscoverySeconds => _activeDiscoverySeconds;
  bool get adEligible => _adEligible;
  bool get adDisplayed => _adDisplayed;
  String? get adId => _adId;
  bool get adLoading => _adLoading;
  String? get adError => _adError;
  bool get shouldShowAd => !_adDisplayed && _adEligible && _adTimerStarted;
  ...
  void startActiveDiscoveryTimer() {
    if (_adTimerStarted && _timer?.isActive == true) return;

    _adTimerStarted = true;
    _paused = false;
    _activeDiscoverySeconds = 0;
    _adEligible = false;
    _adDisplayed = false;
    _adId = null;
    _adLoading = false;
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_paused) return;
      _activeDiscoverySeconds++;

      if (_activeDiscoverySeconds >= config.adIntervalSeconds) {
        _adEligible = true;
        _timer?.cancel();
        notifyListeners();
      } else {
  ...
  void resumeTimer() {
    if (_paused && _adTimerStarted) {
      _paused = false;
      if (_timer?.isActive != true && !_adEligible) {
        _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
          if (_paused) return;
          _activeDiscoverySeconds++;
  ...
  void stopTimer() {
    _timer?.cancel();
    _timer = null;
    _adTimerStarted = false;
    _paused = false;
    notifyListeners();
  }

  /// Mark ad as displayed and reset the active-ad interval
  void markAdDisplayed(Advertisement ad) {
    _adDisplayed = true;
    _adEligible = false;
    _adId = ad.id;
    _adLoading = false;
  ...
  Future<Advertisement?> fetchAd(String userId) async {
    if (!_adEligible || _adDisplayed || _adLoading) return null;
    if (!config.isAdvertisingEnabled) return null;
    ...
    try {
      final (ad, fetchedConfig) = await _repository.fetchAd(userId);
  ...
  void resetAdInterval() {
    ...
    if (!_paused && _timer?.isActive != true) {
      _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (_paused) return;
        _activeDiscoverySeconds++;
  ...
  Future<void> recordImpression(String userId, String adId) async {
    try {
      await _repository.recordEvent(userId, adId, 'impression');
    } catch (e) {
  ...
  Future<void> recordClick(String userId, String adId) async {
    try {
      await _repository.recordEvent(userId, adId, 'click');
    } catch (e) {
  ...
  Future<void> recordReport(String userId, String adId, {String? reason}) async {
    try {
      await _repository.recordEvent(userId, adId, 'report', metadata: {'reason': reason});
    } catch (e) {
      // ignore
    }
  }
  ...
  Future<void> recordHide(String userId, String adId) async {
    try {
      await _repository.recordEvent(userId, adId, 'hide');
    } catch (e) {
      // ignore
    }
  }
...
final adServiceProvider = ChangeNotifierProvider<AdService>((ref) {
  final adRepo = AdRepository();
  return AdService(repository: adRepo);
});
